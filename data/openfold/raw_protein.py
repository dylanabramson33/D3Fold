...
from data.openfold import residue_constants
...
class RawProtein:
    """
    Raw protein structure representation used for generating features via transformations.
    """
...
def from_pdb_string(pdb_str: str, chain_id: Optional[str] = None) -> RawProtein:
...
def get_pdb_headers(prot: RawProtein, chain_id: int = 0) -> Sequence[str]:
    pdb_headers = []
...
def add_pdb_headers(prot: RawProtein, pdb_str: str) -> str:
...
def to_modelcif(prot: RawProtein) -> str:
    """
...
def ideal_atom_mask(prot: RawProtein) -> np.ndarray:
    """Computes an ideal atom mask.

    `RawProtein.atom_mask` typically is defined according to the atoms that are
    reported in the PDB. This function computes a mask according to heavy atoms
    that should be present in the given sequence of amino acids.

    Args:
      prot: `RawProtein` whose fields are `numpy.ndarray` objects.
...
# should move these to different file
